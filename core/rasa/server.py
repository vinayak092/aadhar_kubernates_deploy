...
def obtain_tracker_store(agent: "Agent", conversation_id: Text, request_id: Optional[Text] = UserMessage.DEFAULT_REQUEST_ID, user_id: Optional[Text] = UserMessage.DEFAULT_USER_ID) -> DialogueStateTracker:
    tracker = agent.tracker_store.get_or_create_tracker(conversation_id, request_id, user_id)
    if not tracker:
        raise ErrorResponse(
            409,
            "Conflict",
...
def create_app(
    ...
    async def retrieve_tracker(request: Request, conversation_id: Text):
        ...
        request_id = request.args.get("request_id", UserMessage.DEFAULT_REQUEST_ID)
        user_id = request.args.get("user_id", UserMessage.DEFAULT_USER_ID)

        tracker = obtain_tracker_store(app.agent, conversation_id, request_id, user_id)
    ...
    async def replace_events(request: Request, conversation_id: Text):
        ...
        try:
            tracker = DialogueStateTracker.from_dict(
                conversation_id, request_id, user_id, request.json, app.agent.domain.slots
            )
    ...
    async def execute_action(request: Request, conversation_id: Text):
        ...
        try:
            ...
            await app.agent.execute_action(
                conversation_id, request_id, user_id, action_to_execute, out, policy, confidence
            )
        except Exception as e:
            logger.debug(traceback.format_exc())
            raise ErrorResponse(
    ...
    async def predict(request: Request, conversation_id: Text):
        try:
            ...
            responses = app.agent.predict_next(conversation_id, request_id, user_id)
    ...
    async def add_message(request: Request, conversation_id: Text):
        ...
        request_id = request_params.get("request_id", UserMessage.DEFAULT_REQUEST_ID)
        user_id = request_params.get("user_id", UserMessage.DEFAULT_USER_ID)
        ...
        try:
            user_message = UserMessage(message, None, conversation_id, request_id, user_id, parse_data)
    ...
    async def tracker_predict(request: Request):
        ...
        try:
            tracker = DialogueStateTracker.from_dict(
                sender_id, request_id, user_id, request_params, app.agent.domain.slots
            )
        except Exception as e:
            logger.debug(traceback.format_exc())
            raise ErrorResponse(

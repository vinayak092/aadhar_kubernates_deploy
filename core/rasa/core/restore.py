    ...
    for i, event in enumerate(tracker.events_after_latest_restart()):
        if isinstance(event, UserUttered):
            ...
            await agent.handle_text(
                event.text, sender_id=tracker.sender_id, request_id=tracker.request_id, user_id=tracker.user_id, output_channel=out
            )
            ...
            tracker = agent.tracker_store.retrieve(tracker.sender_id, tracker.request_id, tracker.user_id)
...
def load_tracker_from_json(tracker_dump: Text, domain: Domain) -> DialogueStateTracker:
    ...
    request_id = tracker_json.get("request_id", UserMessage.DEFAULT_REQUEST_ID)
    sender_id = tracker_json.get("user_id", UserMessage.DEFAULT_USER_ID)
    return DialogueStateTracker.from_dict(
        sender_id, request_id, user_id, tracker_json.get("events", []), domain.slots
    )

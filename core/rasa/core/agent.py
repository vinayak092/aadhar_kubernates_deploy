...
class Agent(object):
    ...
    def predict_next(self, sender_id: Text, request_id: Text, user_id: Text, **kwargs: Any) -> Optional[Dict[Text, Any]]:
        """Handle a single message."""

        processor = self.create_processor()
        return processor.predict_next(sender_id, request_id, user_id)
    ...
    async def execute_action(
        ...
        return await processor.execute_action(
            sender_id, request_id, user_id, action, output_channel, self.nlg, policy, confidence
        )

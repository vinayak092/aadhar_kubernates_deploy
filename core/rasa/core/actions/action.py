...
def create_bot_utterance(message: Dict[Text, Any], from_template: Optional[bool] = False) -> BotUttered:
    ...
    bot_message = BotUttered(
        ...
        data={
            ...
            "custom": message.pop("custom", None) if not from_template else DEFAULT_CUSTOM_MESSAGE,
        },
        metadata=message,
    )
...
class ActionUtterTemplate(Action):
    ...
    async def run(self, output_channel, nlg, tracker, domain):
        ...
        return [create_bot_utterance(message, from_template=True)]

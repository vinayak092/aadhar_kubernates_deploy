...
class TrackerWithCachedStates(DialogueStateTracker):
    ...
    def __init__(
        self, sender_id, slots, max_event_history=None, domain=None, is_augmented=False, request_id=UserMessage.DEFAULT_REQUEST_ID, user_id=UserMessage.DEFAULT_USER_ID,
    ):
        super(TrackerWithCachedStates, self).__init__(
            sender_id, request_id, user_id, slots, max_event_history
        )
        self._states = None
        self.domain = domain
    ...
    def copy(self, sender_id: Text = "", 
                   request_id: Text = UserMessage.DEFAULT_REQUEST_ID, 
                   user_id: Text = UserMessage.DEFAULT_USER_ID,) -> "TrackerWithCachedStates":
    ...
    def _process_step(
        ...
        if events:  # small optimization
            ...
            for tracker in incoming_trackers:
                ...
                trackers.append(tracker.copy(new_sender, tracker.request_id, tracker.user_id))
        ...
        for event in events:
            for tracker in trackers:
                if isinstance(
                    ...
                    end_trackers.append(tracker.copy(tracker.sender_id, tracker.request_id, tracker.user_id))
